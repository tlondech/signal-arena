...
def fetch_tennis_scores(api_key: str, league_key: str, days_from: int = 3) -> list[dict]:
    """
    Fetches completed tennis match results from The Odds API /v4/sports/{sport}/scores/.

    Returns a list of:
      {"winner": str, "loser": str, "match_date": datetime}

    `days_from` controls how far back to look (max 3 on most plans).
    Returns [] on any failure — settlement simply skips this tournament.
    """
    url = f"{BASE_URL}/v4/sports/{league_key}/scores/"
    try:
        ...
        logger.debug("Odds API scores: request failed for %s: %s", league_key, e)
        return []

    if response.status_code == 404:
        logger.debug("Odds API scores: league '%s' not found (404).", league_key)
        return []
    if not response.ok:
        logger.debug("Odds API scores: HTTP %d for %s.", response.status_code, league_key)
        return []
    ...
    for event in response.json():
        ...
        # scores is a list of {"name": player_name, "score": sets_won_string}
        score_map = {s["name"]: s["score"] for s in scores if "name" in s and "score" in s}

        try:
            home_sets = int(score_map.get(home, -1))
            away_sets = int(score_map.get(away, -1))
        except (ValueError, TypeError):
            continue

        if home_sets < 0 or away_sets < 0 or home_sets == away_sets:
            continue
        ...
        results.append({"winner": winner, "loser": loser, "match_date": match_date})

    logger.debug("Odds API scores: %d completed result(s) for %s.", len(results), league_key)
    return results
